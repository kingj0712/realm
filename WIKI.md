...
### Templates: what each one is for

The TEMPLATES picker in edit mode offers four curated layouts, each with a different purpose. None is exhaustive; each is a starting point for a particular dashboard mood.

| Template | Purpose |
|----------|---------|
| **Welcome** | First-run guidance only. Pure HeaderTiles, zero entity dependencies. Re-load it any time you want a reminder of what each edit-mode control does. |
| **Smart Home Starter** | Compact practical home layout. Five sections: Home / Climate / Security / Controls / Systems. About half the size of Showcase; good base for a typical install. |
| **Homestead Ops** | Utilities / property / homestead operations. SCADA-leaning, dense, dominated by status telemetry rather than lifestyle cards: weather brief + tanks + generator + water alarms + irrigation + hive + trends + network gear. |
| **Showcase** | Polished example of what a finished Realm dashboard can look like. Eight sections (~25 tiles); not meant for daily use as-is, but for inspiration and as a reference for tile sizing/composition. |
| **/components page** (not a template) | Exhaustive catalog of every tile variant at its registry defaults. Use it to discover tile types when building a layout from scratch. |
...
|-------|------------------|
| **14.6** (current) | **Homestead Ops template polish + template purpose docs**. `homesteadOpsLayout` rewritten to five aligned sections (OPERATIONS / POWER & WATER / LAND & ANIMALS / TRENDS / SYSTEMS), 21 tiles + 5 headers. WeatherRadar dropped from above-the-fold (placeholder dominated without a real iframe URL — users can add it back from the palette). OPERATIONS now leads with Weather + a `WeeklyDigest` SAMPLE brief + a stacked Wind/Humidity column. POWER & WATER replaces the lonely EnergyFlow with a stacked alarm pair (sump high water + water leak). LAND & ANIMALS pairs Irrigation + Beehive with a stacked Mailbox + TrashSchedule column so neither tile floats alone. SYSTEMS section added (Network/SpeedTest/NAS/Homelab at h=8, matching Showcase). New WIKI section 3 sub-block explaining what each template is for: Welcome / Smart Home Starter / Homestead Ops / Showcase / Components page. |
| **14.5** | **Template polish pass**. `smartHomeStarterLayout` rewritten with explicit x/y/w/h to match Showcase's quality bar: five aligned sections (HOME / CLIMATE / SECURITY / CONTROLS / SYSTEMS), no ragged rows from `pack()`, no orphan tiles. Right columns stack two compact tiles where a single tall tile would have left dead space (HOME: SunMoon h=7 + Clock h=4; CLIMATE: UPS Bar h=6 + Outdoor Value h=6; SECURITY: Smoke h=5 + Water h=6). `homesteadOpsLayout` got the same treatment over four sections (OPERATIONS / POWER & WATER / LAND & ANIMALS / TRENDS). Showcase untouched. Comment on the H/T/buildItem helpers updated to reflect that all three curated layouts use them. |
| **14.4** | **Stabilization / docs / QA pass**. Playwright smoke suite covers app shell, fresh-install tabs, Components route, edit-mode entry (pencil + `E`), the grouped banner clusters, and Palette/Templates/Snapshots/Remap/Build-From-HA modals. `tests/smoke.spec.ts` + `playwright.config.ts` with a dedicated dev-server port (4173) so it doesn't collide with interactive `npm run dev` on 5173. CI gained an `e2e` job. Edit-mode banner regrouped into ADD / CONFIGURE / BACKUP / DANGER / DONE clusters so ten actions read calmly; mobile wraps cleanly. Public-demo defaults genericized: `WeatherRadarTile` ships with empty `iframeUrl` and a non-Casco description; `WeeklyDigestTile` default URL is empty and the SAMPLE digest text is generic-flavored. Docs caught up: README current-state line, WIKI tile-count + open-questions cleanup, CLAUDE.md round-13→round-14 callouts, MockHass.ts stale phase-6 TODO removed. New WIKI sections 10.8 (Repo Health / QA), 10.9 (Visual QA Checklist), 10.10 (Dependency upgrade triage). |
