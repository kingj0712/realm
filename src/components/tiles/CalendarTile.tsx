...
export const CalendarTile: FC<CalendarTileProps> = ({ entityId, label, icon, max = 4 }) => {
  ...
  return (
    <>
      <BaseTile label={friendly} icon={icon} pill={`${events.length} UP`} onClick={() => setOpen(true)}>
      ...
      {open && <EntityDetailModal entityId={entityId} title={friendly} pill={`${events.length} UP`} onClose={() => setOpen(false)} />}
    </>
  );
};
